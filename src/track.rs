...
use crate::mp4box::{
    ...
    opus::OpusBox, smhd::SmhdBox, stco::StcoBox, stsc::StscEntry, stss::StssBox,
    stts::SttsEntry, tx3g::Tx3gBox, vmhd::VmhdBox, vp09::Vp09Box,
};
use crate::*;
...
impl Mp4Track {
    ...
    pub fn opus_config(&self) -> Result<OpusConfig> {
        if let Some(ref opus) = self.trak.mdia.minf.stbl.stsd.opus {
            ...
            Ok(OpusConfig {
                ...
                stream_count: dops.channel_mapping_table.as_ref().map(|t| t.stream_count),
                coupled_count: dops.channel_mapping_table.as_ref().map(|t| t.coupled_count),
                channel_mapping: dops
                    .channel_mapping_table
                    .as_ref()
                    .map(|t| t.channel_mapping.clone()),
            })
        } else {
